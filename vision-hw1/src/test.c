...
void hybrid_image()
{
    ...
    image dum = load_image("data/dumbledore.png");
    image fron = make_gaussian_filter(6);
    image fdum = make_gaussian_filter(8);
    image lron = convolve_image(ron, fron, 1);
    image ldum = convolve_image(dum, fdum, 1);
    image hron = sub_image(ron, lron);
    image ronbledore = add_image(hron, ldum);
...
void colorize_representation()
{
    image im = load_image("data/dog.jpg");
    image colorized = colorize_sobel(im);
    save_image(colorized, "sobel");
    free_image(im);
    free_image(colorized);
}
...
void run_tests()
{
    ...
    // hybrid image and colorize sobel
    hybrid_image();
    colorize_representation();
}
